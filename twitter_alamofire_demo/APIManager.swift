...
class APIManager: SessionManager {
    ...
    static let consumerKey = "vdJZZNuGbd2QYIQiruutT7qXS"
    static let consumerSecret = "PcZrPkZKWJEBVWv3Uyc0wLYpUnxKNH2RC4mOCoz6F8ybj9XzyE"
    ...
    func getMoreTimelineTweets(with tweetId: Int64, completion: @escaping ([Tweet]?, Error?) -> ()) {
...
//        }
        ...
        request(URL(string: "https://api.twitter.com/1.1/statuses/user_timeline.json")!, method: .get)
            ...
            .responseJSON { (response) in
                ...
                let data = NSKeyedArchiver.archivedData(withRootObject: tweetDictionaries)
                UserDefaults.standard.set(data, forKey: "personaltimeline_tweets")
                UserDefaults.standard.synchronize()
    ...
    func favorite(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        ...
        twitterRequest(withURL: urlString, forTweet: tweet, completion: completion)
    }
    ...
    // Network Request Action to Twitter Api
    private func twitterRequest(withURL urlString: String, forTweet tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
